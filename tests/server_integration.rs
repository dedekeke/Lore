...
async fn test_get_task_stats() {
    ...
    let task = server
        .start_task("stats task".into(), None)
        .await
        .unwrap();
    let task_id = extract_json(&task)["task_id"]
        .as_str()
        .unwrap()
        .to_string();
...
async fn test_abandon_task() {
    ...
    let task = server
        .start_task("abandon me".into(), None)
        .await
        .unwrap();
    let task_id = extract_json(&task)["task_id"]
        .as_str()
    ...
    let list = server
        .list_tasks(Some("abandoned".into()))
        .await
        .unwrap();
...
async fn test_update_rule() {
    ...
    let updated = server
        .update_rule(rule_id.clone(), Some("lesson".into()), Some("updated content".into()))
        .await
        .unwrap();
...
async fn test_log_context_wipe() {
    ...
    let result = server
        .log_context_wipe(task_id, 10000, None)
        .await
        .unwrap();
    let json = extract_json(&result);
